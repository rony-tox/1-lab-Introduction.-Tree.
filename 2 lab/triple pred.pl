...
read_list([HeadListTailList])-read(X), X = end, HeadList is X, read_list(TailList).
read_list([])-!.
% write_list(+List) - напечатать список
write_list([])-!.
write_list([HT])-write(H), nl, write_list(T).
...
% check_max_element_in_list0 - предикат для определения, является ли элемент по указанному индексу, глобальным максимумом.
...
min_in_interval_in_list([HeadTail], From, To, CurMin, _, Result)- 
...
min_in_interval_in_list([HeadTail], From, To, CurMin, CurRes, Result)-
    check_interval(Head, From, To),
    Head == CurMin,
...
min_in_interval_in_list([_Tail], From, To, CurMin, CurRes, Result)- min_in_interval_in_list(Tail, From, To, CurMin, CurRes, Result), !.
% check_count_min_in_interval0 - найти количество минимальных элементов на интервале внутри списка
...
max_in_list([HeadListTailList], CurRes, Result)- var(CurRes), CurRes is HeadList, max_in_list(TailList, CurRes, Result), !.
max_in_list([HeadListTailList],CurRes, Result)- HeadList  CurRes, max_in_list(TailList, HeadList, Result), !.
max_in_list([_TailList], CurRes, Result)- max_in_list(TailList, CurRes, Result), !.

% check_max_in_interval0 - проверить наличие максимального элемента списка в интервале
